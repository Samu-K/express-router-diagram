...
describe("Playwright E2E Tests", () => {
	...
	beforeAll(() => {
		return new Promise((resolve) => {
			...
			expressApp.get("/api/users", (req, res) => res.json({ users: [] }));
			expressApp.post("/api/users", (req, res) => res.json({ success: true }));
	...
	it("should run Playwright tests", async () => {
		...
		if (process.env.CI && !process.env.PLAYWRIGHT_BROWSERS_PATH) {
			console.log(
				"Skipping Playwright tests in CI environment without browsers",
			);
			return;
		}
