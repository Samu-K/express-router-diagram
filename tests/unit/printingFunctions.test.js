...
vi.mock("../../src/utils/colorUtils", () => {
	...
	const colorizeMethodsString = vi.fn(
		(methods) => `ColorizedString ${methods}`,
	);
...
import diagramUtils from "../../src/utils/diagramUtils";
import hierarchyUtils from "../../src/utils/hierarchyUtils";
...
describe("printingFunctions", () => {
	...
	describe("printAppRoutes", () => {
		...
		it("should log header when logToConsole is true", () => {
			...
			printingFunctions.printAppRoutes(
				app,
				{ logToConsole: true },
			...
			expect(consoleLogSpy).toHaveBeenCalledWith(
				expect.stringContaining("EXPRESS ROUTES"),
			);
		});
		...
		it("should return the routes array", () => {
			...
			const result = printingFunctions.printAppRoutes(
				app,
				{},
				extractRoutesFromApp,
			);
	...
	describe("printRoutesInMiddleware", () => {
		...
		it("should print simplified output in test environment", () => {
			...
			expect(consoleLogSpy).toHaveBeenCalledWith(
				"Hierarchical diagram (simplified for tests)",
			);
		});
