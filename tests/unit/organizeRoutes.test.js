...
describe("organizeRoutesHierarchy", () => {
	...
	it("should handle routes with deep nesting", () => {
		...
		expect(
			Array.isArray(hierarchy.api.users[":userId"][0].posts[":postId"]),
		).toBe(true);

		// Check that the first item in the array has comments
		expect(
			hierarchy.api.users[":userId"][0].posts[":postId"][0],
		).toHaveProperty("comments");

		// Check that comments is an array
		const comments =
			hierarchy.api.users[":userId"][0].posts[":postId"][0].comments;
		...
		expect(comments[0].path).toContain(
