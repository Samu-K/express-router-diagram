...
const printingFunctions = {
	 ...
	 */
	printAppRoutes(app, options = {}, extractRoutesFromApp) {
		...
		const filteredRoutes = filterUtils.filterRoutes(
			routes,
		...
		if (config.logToConsole) {
			...
			console.log(
				`${colorUtils.colors.cyan}==============${colorUtils.colors.reset}\n`,
			);

			if (config.hierarchical !== false) {
				// Generate hierarchical diagram
				const hierarchy =
					hierarchyUtils.organizeRoutesHierarchy(filteredRoutes);

				// Use the fixed printHierarchyToString function
				const hierarchyString = hierarchyUtils.printHierarchyToString(
					hierarchy,
					0,
					{ useColors: true },
				);
		...
		if (config.hierarchical !== false) {
			...
			if (isTestEnv) {
				...
				const hierarchyString =
					hierarchyUtils.printHierarchyToString(hierarchy);
				console.log(hierarchyString);
			}
		} else {
			// Generate flat list diagram
