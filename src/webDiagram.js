...
function setupWebRoute(app, routes, webRoutePath = "/express-routes") {
  ...
  const webRoute =
    webRoutePath && !webRoutePath.startsWith("/") ? `/${webRoutePath}` : webRoutePath;
  ...
  app.get(webRoute, (_, res) => {
    ...
    try {
      ...
      res.status(500).send("Error generating D3 visualization: ", error.message);
    }
  });
...
function generateRoutesOnStartup(app) {
  ...
  console.log(`Generated ${routes.length} routes for web visualization on startup`);
  return routes;
}
