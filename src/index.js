...
function routerDiagramMiddleware(options = {}) {
	...
	try {
		...
		console.error(
			"Express is required for the middleware. Make sure it is installed.",
		);
	}
	...
	return (req, res, next) => {
		...
		if (
			config.generateWeb &&
			(req.path === webRoutePath || req.path === dataRoutePath)
		) {
			// Use pre-generated routes if provided, otherwise extract them
			let routes =
				config.preGeneratedRoutes ||
				req.app._routerDiagramRoutes ||
				extractRoutesFromApp(req.app);
			...
			if (req.path === webRoutePath) {
				try {
					...
					if (!fs.existsSync(templatePath)) {
						console.error(
							"D3 diagram template file not found at:",
							templatePath,
						);
						return res
							.status(500)
